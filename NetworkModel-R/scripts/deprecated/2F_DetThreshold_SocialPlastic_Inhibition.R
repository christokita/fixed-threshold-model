...
# Base parameters
Ns             <- c(1, 2, 4, 5,6, 8, 12, 16) #vector of number of individuals to simulate
m              <- 2 #number of tasks
...
c              <- 10 #social reinforcer of threshold learning
d              <- 10 #social preventer of threshold forgetting
Averaging      <- TRUE #shoudl social interactions be done in absolute or averaged terms?
lowerThresh    <- 0
upperThresh    <- 100

# Social Network Parameters
p              <- 0.4 #probability of interacting with other individuals
q              <- 1.5 #probability of interacting with individual in same state relative to others
...
for (i in 1:length(Ns)) {
  ...
  for (sim in 1:reps) {
    ...
    # Seed upper and lower bounds on plasticity
    threshLowBound <- threshMat - ThreshBound
    threshHighBound <- threshMat + ThreshBound
    ...
    for (t in 1:gens) {
      ...
      threshMat <- adjustThresholdsSocialInhibition_Bound(ThresholdMatrix = threshMat, 
                                                    X_sub_g = X_g, 
                                                    phi = phi, 
                                                    phiMult = phiMult, 
                                                    c = c, 
                                                    d = d, 
                                                    SocialNetwork = g_adj, 
                                                    lowerThresh = threshLowBound, 
                                                    upperThresh = threshHighBound, 
                                                    Average = Averaging)
