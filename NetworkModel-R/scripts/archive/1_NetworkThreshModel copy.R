...
epsilon        <- 0.2 #weighting of social information relative to task demand information (must be between 0 and 1) 
...
ThreshSD       <- ThreshM * 0.2 #population threshold standard deviations !!Change!!
InitialStim    <- c(0, 0) #intital vector of stimuli
StimRates      <- c(0.8, 0.8) #vector of stimuli increase rates  
ExhaustM       <- 5 #population exhaustion threshold mean !!Change!!
ExhaustSD      <- ExhaustM * 0.2 #populution exhaustion threshold standard deviation !!Change!!
ExhaustRate    <- 0.8 #rate of exhaustion from performing task  !!Change!!
...
for (i in 1:length(Ns)) {
  ...
  for (sim in 1:reps) {
    ...
    # Social threshold
    socialThreshMat <- seedExhaustThresh(n = n, 
                                         ExhaustMean = 0.5, 
                                         ExhaustSD = 0.2)
