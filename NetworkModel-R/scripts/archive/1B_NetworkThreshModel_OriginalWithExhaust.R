...
# Base parameters
Ns             <- c(2, 4, 16) #vector of number of individuals to simulate
m              <- 2 #number of tasks
gens           <- 2000 #number of generations to run simulation 
corrStep       <- 100 #number of time steps for calculation of correlation 
reps           <- 5 #number of replications per simulation (for ensemble) !!Change!!
...
ExhaustM       <- 10 #population exhaustion threshold mean !!Change!!
...
for (i in 1:length(Ns)) {
  ...
  for (sim in 1:reps) {
    ...
    for (t in 1:gens) {
      ...
      # X_g <- updateTaskPerformance_og_exhaust(P_sub_g    = P_g,
      #                                         TaskMat    = X_g,
      #                                         ExhaustStim = exhaustStim,
