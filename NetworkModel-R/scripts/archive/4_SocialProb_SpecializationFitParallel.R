...
threshSlopes   <- c(1, 2, 3, 4, 5, 7, 10, 15, 20, 25, 30) #exponent parameter for threshold curve shape  
nSlope         <- 2
alpha          <- m #efficiency of task performance
quitP          <- 0.2 #probability of quitting task once active
phi            <- 0.005 #learning/forgetting of threshold
gamma          <- 0.005   #bias towards learning of task
cs             <- seq(0, 0.5, 0.05) #social reinforcer of threshold learning
...
iteration <- 100 / (length(threshSlopes) * length(cs) * length(Ns))
...
improveSpec <- sfLapply(1:length(threshSlopes),function(k) {
  ...
  cDiff <- data.frame(PercIncrease = NULL, SlopeIncrease = NULL, SpecLarge = NULL, SpecSmall = NULL, c = NULL, threshSlope = NULL)
  ...
  for (z in 1:length(cs)) {
    ...
    cDiff <- rbind(cDiff, CorrMeanDiff)
  }
