...
phis           <- c(0, 0.005, 0.01, 0.015, 0.02, 0.03, 0.05, 0.1) #learning/forgetting of threshold
gamma          <- 0.01 #bias towards learning of task !!Change!!
lowerThresh    <- 0
upperThresh    <- 100
...
for (k in 1:length(threshSlopes)) {
  ...
  for (z in 1:length(phis)) {
    ...
    for (i in 1:length(Ns)) {
      ...
      print(paste0("DONE: N = ", n, ", Phi = ", phi, ", N-Slope = ", threshSlope))
