...
gamma          <- 0.01 #learning/forgetting of threshold
phi            <- 0.005 #learning/forgetting of threshold
lowerThresh    <- 0
upperThresh    <- 100
...
for (i in 1:length(Ns)) {
  ...
  for (sim in 1:reps) {
    ...
    for (t in 1:gens) {
      ...
      thresh <- as.data.frame(threshMat)
      thresh$individual <- row.names(thresh)
      thresh <- melt(thresh, id.vars = "individual")
      names(thresh) <- c("individual", "task", "threshold")
      thresh$t <- t
      thresh$n <- n
      thresh$replicate <- sim
      threshDF <- rbind(threshDF, thresh)
...
# Plot
gg_thresh <- ggplot(data = threshDF, 
                    aes(x = t, y = threshold, colour = individual)) +
...
stims <- stims %>% 
  ...
  select(s1, s2, n, replicate, t)
stims <- melt(stims, id.vars = c("n", "replicate", "t"))
names(stims) <- c("n", "replicate", "t", "task", "stim")

gg_stim <- ggplot(data = stims, 
                    aes(x = t, y = stim)) +
  geom_line() +
  theme_classic() +
  facet_grid(n ~ task) +
...
png("output/OtherFigures/ThresholdsOverTime/SelfDelta06Gamma01.png", width = 8, height = 4, units = "in", res = 300)
multiplot(gg_thresh, gg_stim, cols = 2)
dev.off()
...
#   ggtitle(paste("Self-Reinforcing Thresholds,\nGamma =", gamma, "Phi = ", phi, "Sigma =", sigma[1])) +
#   theme(legend.position = "none",
#         axis.text = element_text(size = 6))
# gg_corrtime
# 
# ggsave(filename = paste0("output/OtherFigures/CorrTime_Gamma", gamma, "Sigma", sigma[1], "Self.png"), width = 5, height = 5, units = "in", dpi = 300)
# 
# 
