...
gamma          <- 0.005 #learning/forgetting of threshold
phi            <- 0.005 #learning/forgetting of threshold
lowerThresh    <- 0
upperThresh    <- 100
c              <- 0.2 #social reinforcer of threshold learning
d              <- 0.2 #social preventer of threshold forgetting
Averaging      <- FALSE #shoudl social interactions be done in absolute or averaged terms?


# Social Network Parameters
p              <- 0.4 #probability of interacting with individual in other states
...
gg_corrtime <- ggplot(data = Corrs, 
  ...
  ggtitle(paste("Socially-Reinforced Thresholds,\nGamma =", gamma, "c = ", c)) +
  theme(legend.position = "none",
        axis.text = element_text(size = 6))
gg_corrtime

ggsave(filename = paste0("output/OtherFigures/CorrTime_C", c, "Social.png"), width = 5, height = 5, units = "in", dpi = 300)
