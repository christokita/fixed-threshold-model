...
# Set variable
filename <- "ProbSocialInhibit__Sum_Delta06Sigma00Phi001c015d015_connectP04Bias2.5"
...
resampledCI <- resampleCI(TaskDistList = groups_taskDist, resampleNumber = 1000)
resampledCI <- as.data.frame(resampledCI)
resampledCI[is.na(resampledCI)] <- 0

resampledCIexp <- resampleCI_experiment(ExperimentalData = yukoData, resampleNumber = 1000)
resampledCIexp <- as.data.frame(resampledCIexp)
resampledCIexp[is.na(resampledCIexp)] <- 0
...
gg_var <- ggplot() +
  ...
  geom_ribbon(data = resampledCI,
              aes(x = n, ymin = SD_avg - SD_95Dist, ymax = SD_avg + SD_95Dist),
              fill = "#93cdff",
              alpha = 0.4) +
  geom_line(data = resampledCIexp,
            aes(x = n, y = SD_avg),
            colour = "#93cdff",
            size = 0.8) +
  geom_ribbon(data = resampledCIexp,
              aes(x = n, ymin = SD_avg - SD_95Dist, ymax = SD_avg + SD_95Dist),
              fill = "#93cdff",
              alpha = 0.4) +
  # Add data
...
# Plot variance and mean by group size
taskVarMeanShow <- taskVarMean %>% 
  filter(Source == "Model")
taskVarMeansShow <- taskVarMeans %>% 
  filter(Source == "Model")
gg_var_ex <- ggplot() +
  # Add resampled CI
  geom_line(data = resampledCI,
  ...
  geom_ribbon(data = resampledCI,
              ...
              alpha = 0.4) +
  # Add data
  geom_point(data = taskVarMeanShow, 
             aes(x = n, y = SD),
             colour = "black",
             size = 0.8,
             alpha = 0.4) +
  ...
  scale_y_continuous(breaks = seq(0, 1, 0.025)) +
  # Mean and SE portion of plot
  geom_errorbar(data = taskVarMeansShow, 
                aes(x = n, ymin = SDMean - SDSE, ymax = SDMean + SDSE, width = 1.5),
                colour = "black") +
  geom_point(data = taskVarMeansShow, 
             aes(x = n, y = SDMean),
             colour = "black",
             size = 2) +
  geom_line(data = taskVarMeansShow,
            aes(x = n, y = SDMean),
            colour = "black") +
  theme(legend.position = "none")
...
gg_corr <- ggplot() +
  ...
  geom_errorbar(data = taskCorrMeans, 
                aes(x = n, ymin = SpecMean - SpecCI, ymax = SpecMean + SpecCI, colour = Source, width = 1.5),
                position = position_dodge(width = 1)) +
