...
c              <- 0.3 #social reinforcer of threshold learning
d              <- 0.3 #social preventer of threshold forgetting
...
for (i in 1:length(Ns)) {
  ...
  for (sim in 1:reps) {
    ...
    for (t in 1:gens) {
      ...
      threshMat <- adjustThresholdsSocial_bound(ThresholdMatrix = threshMat, 
                                                X_sub_g = X_g, 
                                                phi = phi, 
                                                phiMult = phiMult, 
                                                c = c, 
