...
####################
# load
load("output/ParameterExploration/Rdata/SelfDelta06_GammaSlopeExploration.Rdata")
improve <- improve %>% 
...
##### Slope #####
# Fit surface
spec.loess <- loess(relativeSlope ~ gamma * threshSlope, data = improve, degree = 2, span = 0.25)
spec.fit <- expand.grid(list(gamma = seq(0, max(improve$gamma), (max(improve$gamma) - min(improve$gamma)) / 1000), 
...
# Graph 
gg_slope <- ggplot(spec.fit, aes(x = gamma, y = threshSlope, fill = spec)) +
  geom_tile() +
  theme_bw() +
...
##### N = 2 #####
# Fit surface
spec.loess <- loess(relativeSmall ~ gamma * threshSlope, data = improve, degree = 2, span = 0.25)
spec.fit <- expand.grid(list(gamma = seq(0, max(improve$gamma), (max(improve$gamma) - min(improve$gamma)) / 1000), 
                             threshSlope = seq(1, max(improve$threshSlope), (max(improve$threshSlope) - min(improve$threshSlope)) / 1000)))
z <- predict(spec.loess, newdata = spec.fit)
...
# Graph 
gg_small <- ggplot(spec.fit, aes(x = gamma, y = threshSlope, fill = spec)) +
  geom_tile() +
  theme_bw() +
  geom_hline(mapping = aes(yintercept = 2), 
             linetype = "dashed", size = 0.3) +
...
##### N = 16 #####
# Fit surface
spec.loess <- loess(relativeLarge ~ gamma * threshSlope, data = improve, degree = 2, span = 0.25)
spec.fit <- expand.grid(list(gamma = seq(0, max(improve$gamma), (max(improve$gamma) - min(improve$gamma)) / 1000), 
                             threshSlope = seq(1, max(improve$threshSlope), (max(improve$threshSlope) - min(improve$threshSlope)) / 1000)))
z <- predict(spec.loess, newdata = spec.fit)
...
# Graph 
gg_large <- ggplot(spec.fit, aes(x = gamma, y = threshSlope, fill = spec)) +
  geom_tile() +
  theme_bw() +
  geom_hline(mapping = aes(yintercept = 2), 
             linetype = "dashed", size = 0.3) +
...
##### Fit #####
# Fit surface
spec.loess <- loess(fit ~ gamma * threshSlope, data = improve, degree = 2, span = 0.25)
spec.fit <- expand.grid(list(gamma = seq(0, max(improve$gamma), (max(improve$gamma) - min(improve$gamma)) / 1000), 
                             threshSlope = seq(1, max(improve$threshSlope), (max(improve$threshSlope) - min(improve$threshSlope)) / 1000)))
z <- predict(spec.loess, newdata = spec.fit)
...
# start building large dataframe for calculating total fit
# totalfit <- totalfit %>%
#   mutate(spec = (abs(slope.fit) + abs(small.fit) + abs(large.fit)) / 3)
# spec.fit <- totalfit
# Find which z are within range 0.95 - 1.05? (relative to data)
# Try custom color
...
# Graph 
gg_fit <- ggplot(spec.fit, aes(x = gamma, y = threshSlope, fill = spec)) +
  geom_tile() +
  theme_bw() +
  geom_hline(mapping = aes(yintercept = 2), 
             linetype = "dashed", size = 0.3) +
...
##### Stm differences #####
# Fit surface
stim.loess <- loess(StimDiffSmall ~ gamma * threshSlope, data = improve, degree = 2, span = 0.25)
stim.fit <- expand.grid(list(gamma = seq(0, max(improve$gamma), (max(improve$gamma) - min(improve$gamma)) / 1000), 
                             threshSlope = seq(1, max(improve$threshSlope), (max(improve$threshSlope) - min(improve$threshSlope)) / 1000)))
...
# Graph 
gg_stimSmall <- ggplot(stim.fit, aes(x = gamma, y = threshSlope, fill = stim)) +
  geom_tile() +
  theme_bw() +
  geom_hline(mapping = aes(yintercept = 2), 
             linetype = "dashed", size = 0.3) +
...
# Fit surface
stim.loess <- loess(StimDiffLarge ~ gamma * threshSlope, data = improve, degree = 2, span = 0.25)
stim.fit <- expand.grid(list(gamma = seq(0, max(improve$gamma), (max(improve$gamma) - min(improve$gamma)) / 1000), 
                             threshSlope = seq(1, max(improve$threshSlope), (max(improve$threshSlope) - min(improve$threshSlope)) / 1000)))
z <- predict(stim.loess, newdata = stim.fit)
...
# Graph 
gg_stimLarge <- ggplot(stim.fit, aes(x = gamma, y = threshSlope, fill = stim)) +
  geom_tile() +
  theme_bw() +
  geom_hline(mapping = aes(yintercept = 2), 
             linetype = "dashed", size = 0.3) +
...
##### Plot #####
# output
png(paste0("output/ParameterExploration/Plot/", filename, "FitAndStim.png"), width = 6, height = 2, units = "in", res = 300)
...
# Graph 
gg_diff <- ggplot(diff.fit, aes(x = gamma, y = threshSlope, fill = spec)) +
  geom_tile() +
  theme_bw() +
  geom_hline(mapping = aes(yintercept = 2), 
             linetype = "dashed", size = 0.3) +
